...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    if (!target.is_unsolve()) {
      ...
      // 绿：当前帧模型状态的整符；蓝：预测时刻的整符。亮色是当前 target 槽。
      auto_buff::draw_whole_buff(img, solver, target, {0, 140, 0}, {0, 255, 0}, 4, "NOW");
      auto_buff::draw_whole_buff(
        img, solver, target_copy, {140, 60, 0}, {255, 100, 0}, 4, "PREDICT");
