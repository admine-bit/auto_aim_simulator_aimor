  ...
  const std::string & config_path, const std::string & can, const std::string & output_folder)
{
  ...
  while (true) {
    ...
    auto q = gimbal.q(timestamp);// 获取该时间戳对应的云台姿态四元数- 1ms
    ...
    auto success = cv::findCirclesGrid(img, cv::Size(7, 6), centers_2d);  // 默认是对称圆点图案
    cv::drawChessboardCorners(img_with_ypr, cv::Size(7, 6), centers_2d, success);  // 显示识别结果
