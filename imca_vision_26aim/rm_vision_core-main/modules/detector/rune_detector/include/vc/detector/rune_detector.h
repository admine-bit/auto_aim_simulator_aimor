...
public:
    ...
    /// 本帧由真实轮廓构造的 R 中心；强制构造与掉帧重投影均为 nullopt。
    std::optional<cv::Point2f> observedRuneCenter() const { return observed_rune_center_; }

    /// 本帧检测实际使用的二值图；由上层调试界面负责复制和显示。
    const cv::Mat & binaryImage() const noexcept { return binary_image_; }
     ...
     *
     * @note 公开是为了让调试界面在不运行整套检测的前提下预览阈值效果，
     *       复用同一份二值化实现，避免上层自己抄一遍导致行为漂移。
     */
...
private:
    std::optional<cv::Point2f> observed_rune_center_;
