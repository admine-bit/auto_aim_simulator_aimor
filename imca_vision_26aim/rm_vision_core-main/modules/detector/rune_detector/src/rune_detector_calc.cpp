...
{
    const size_t max_count = static_cast<size_t>(rune_detector_param.MAX_PENDING_STRUCK_COUNT);
    if (inactive_targets.size() > max_count)
    {
        // 按面积降序排列，保留面积最大的前 max_count 个未激活靶心
        sort(inactive_targets.begin(), inactive_targets.end(), [](const FeatureNode_ptr &t1, const FeatureNode_ptr &t2)
             { return t1->getImageCache().getContours().front()->minAreaRect().size.area() > t2->getImageCache().getContours().front()->minAreaRect().size.area(); });
        inactive_targets.resize(max_count);
    }
                                  ...
                                  const vector<FeatureNode_ptr> &active_fans)
{
    ...
    {
        // 当没有参考特征时，按面积降序排列并保留最多 MAX_PENDING_STRUCK_COUNT 个
        sort(inactive_fans.begin(), inactive_fans.end(), [](const FeatureNode_ptr &f1, const FeatureNode_ptr &f2)
             { return getRotatedRectArea(f1) > getRotatedRectArea(f2); });
        const size_t max_count = static_cast<size_t>(rune_detector_param.MAX_PENDING_STRUCK_COUNT);
        if (inactive_fans.size() > max_count)
            inactive_fans.resize(max_count);
        return false;
    }
    ...
    // 按距离排序，筛掉距离其它特征过远的扇叶
    vector<pair<FeatureNode_ptr, double>> sorted_fans(fan_distance.begin(), fan_distance.end());
    sort(sorted_fans.begin(), sorted_fans.end(), [](const auto &a, const auto &b)
         { return a.second < b.second; });

    vector<FeatureNode_ptr> valid_fans;
    for (auto &[fan, dist] : sorted_fans)
    {
        float max_dis = 0;
        for (auto &inactive_target : inactive_targets)
            max_dis = max(max_dis, getDist(getCenter(fan), getCenter(inactive_target)));
        for (auto &active_target : active_targets)
            max_dis = max(max_dis, getDist(getCenter(fan), getCenter(active_target)));
        for (auto &active_fan : active_fans)
            max_dis = max(max_dis, getDist(getCenter(fan), getCenter(active_fan)));

        if (max_dis <= max(fan->getImageCache().getHeight(), fan->getImageCache().getWidth()) * rune_fan_param.INACTIVE_MAX_DISTANCE_RATIO)
            valid_fans.push_back(fan);
    }
    ...
    // 保留最多 MAX_PENDING_STRUCK_COUNT 个未激活扇叶
    const size_t max_count = static_cast<size_t>(rune_detector_param.MAX_PENDING_STRUCK_COUNT);
    if (valid_fans.size() > max_count)
        valid_fans.resize(max_count);

    inactive_fans = std::move(valid_fans);
    return true;
}
