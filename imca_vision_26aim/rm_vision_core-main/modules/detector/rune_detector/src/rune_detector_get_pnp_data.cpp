...
inline vector<float> getRuneDeviation(const std::vector<RuneFeatureComboConst> &runes)
{
    ...
    for (const auto &feature_group : temp_runes)
    {
        ...
        {
            VC_WARNING_INFO("getRuneDeviation skip frame: target/fan/center is nullptr");
            return {};
        }
        float angle;
        if (target != nullptr)
        ...
        {
            VC_WARNING_INFO("getRuneDeviation skip frame: angle is not finite");
            return {};
        }
        // 将角度限定在 0 ~ 360 之间
    ...
    for (auto &[target, center, fan] : temp_runes)
    {
        ...
        {
            VC_WARNING_INFO("getRuneDeviation skip frame: target and fan are nullptr");
            return {};
        }
    }
    ...
    for (auto &feature_group : temp_runes)
    {
        ...
        if (min_delta_idx == -1)
        {
            VC_WARNING_INFO("getRuneDeviation skip frame: min_delta_idx is -1");
            return {};
        }
    ...
    if(rune_idx.size() > 5)
    {
        VC_WARNING_INFO("getRuneDeviation skip frame: rune_idx size is greater than 5");
        return {};
    }
                         ...
                         std::vector<float> &point_weights)
{
    if (runes.empty())
    {
        VC_WARNING_INFO("getPnpPoints skip frame: matched features is empty");
        return false;
    }

    vector<float> deviation_angles = getRuneDeviation(runes);                                       // 获取神符偏移角度
    if (deviation_angles.size() != runes.size())
    {
        VC_WARNING_INFO("getPnpPoints skip frame: invalid deviation size %zu (runes=%zu)",
                        deviation_angles.size(), runes.size());
        return false;
    }
    ...
    for (auto &feature_group : runes)
    {
        ...
        if ((target == nullptr && fan == nullptr) || center == nullptr)
        {
            VC_WARNING_INFO("getPnpPoints skip frame: target/fan/center is nullptr");
            return false;
        }
    ...
    {
        VC_WARNING_INFO("getPnpPoints skip frame: 2D/3D/weight size mismatch");
        return false;
    }
