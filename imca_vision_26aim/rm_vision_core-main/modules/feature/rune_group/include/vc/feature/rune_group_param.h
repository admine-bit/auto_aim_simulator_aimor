...
struct RuneGroupParam
{
    ...
    //! 两帧之间的最大 roll 偏差 (度)
    double MAX_ROLL_DEVIATION = 20.0;

    //! 两帧之间的最大 x 偏差 (单位：毫米)
    double MAX_X_DEVIATION = 2000.0;

    //! 两帧之间的最大 y 偏差 (单位：毫米)
    double MAX_Y_DEVIATION = 2000.0;

    //! 两帧之间的最大 z 偏差 (单位：毫米)
    double MAX_Z_DEVIATION = 2000.0;
    ...
    //! 最大待击打（PENDING_STRUCK）目标数量
    //! 小能量机关天然只有1个inactive靶心，设为2不影响；大能量机关需要保留2个
    int MAX_PENDING_STRUCK_COUNT = 2;
    ...
    YML_INIT(
        ...
        YML_ADD_PARAM(MAX_PENDING_STRUCK_COUNT););
};
