...
: classifier_(config_path), detector_(config_path), debug_(debug)
{
  ...
  // 模型路径由编译后端决定，分类和装甲板后处理保持主分支实现。
  engine_ = make_infer_engine(config_path, "yolov8_model_path", "yolov8_engine_path", 416, 416);
}
...
std::list<Armor> YOLOV8::detect(const cv::Mat & raw_img, int frame_count)
{
  ...
  // 后端完成 letterbox、预处理和推理，scale 用于恢复原图坐标。
  double scale = 1.0;
  auto output = engine_->infer_letterbox(bgr_img, scale);
