...
#[repr(C)]
struct MessageTypeSupportView {
    _typesupport_identifier: *const c_char,
    data: *const c_void,
    _func: rosidl_message_typesupport_handle_function,
}

impl TypeSupport {
    pub unsafe fn introspection(&self) -> Introspection<'_> {
        let support = &self.0 as *const CTypeSupport as *const MessageTypeSupportView;
        let members = ((*support).data as *const MessageMembers).as_ref().unwrap();
